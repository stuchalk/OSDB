...
Clazz.load (["JU.P3"], "JS.SpaceGroupFinder", ["JU.BS", "$.Lst", "JS.SpaceGroup", "$.SymmetryOperation", "JU.BSUtil", "$.SimpleUnitCell", "JV.FileManager"], function () {
...
bsOps.setBits (1, JS.SpaceGroupFinder.OP_COUNT);
if (this.nAtoms == 0) {
...
opsChecked.set (0);
var hasC1 = false;
for (var iop = bsOps.nextSetBit (1); iop > 0 && bsGroups.cardinality () > 0; iop = bsOps.nextSetBit (iop + 1)) {
...
bs1.andNot (targets);
this.dumpBasis (JS.SpaceGroupFinder.bsGroupOps[isg], bs1, bsPoints0);
map.put ("name", name);
map.put ("basis", basis);
...
Clazz.defineMethod (c$, "filterGroups", 
 function (bsGroups, uc) {
var params = uc.getUnitCellParams ();
var isOrtho = false;
var isTet = false;
var isTri = false;
var isRhombo = false;
...
Clazz.defineMethod (c$, "getGroupsWithOp", 
 function (opXYZ0) {
var groups = null;
for (var i = JS.SpaceGroupFinder.opXYZ.length; --i >= 0; ) {
if (JS.SpaceGroupFinder.opXYZ[i].equals (opXYZ0)) {
groups = JS.SpaceGroupFinder.bsOpGroups[i];
System.out.println ("found for " + opXYZ0 + " " + groups);
break;
}}
if (groups != null) {
var ret =  new Array (groups.cardinality ());
for (var p = 0, i = groups.nextSetBit (0); i >= 0; i = groups.nextSetBit (i + 1)) {
ret[p++] = JS.SpaceGroup.getID (i);
}
return ret;
}return null;
}, "~S");
...
if (op == null) {
JS.SpaceGroupFinder.ops[iop] = op =  new JS.SymmetryOperation (null, null, 0, iop, false);
...
JS.SpaceGroupFinder.rdr = JV.FileManager.getBufferedReaderForResource (vwr, me, "J/symmetry/", "sg/" + fileName);
if (list == null) {
var l =  new JU.Lst ();
var line;
